...
const globalJsString = `const navLinks = Array.from(document.querySelectorAll("nav .chip"));
...
fs.writeFileSync('assets/js/global.js', globalJsString);
...
files.forEach(file => {
  ...
  if (jsStartMatch) {
    let startIndex = jsStartMatch.index;
    let endStr = 'const getInitialLang = () => {\n';
    let getInitIndex = content.indexOf(endStr, startIndex);
    if (getInitIndex !== -1) {
  ...
  // Remove theme transition logic if we move it to global? Wait, I didn't move startThemeTransition to globalJs in this version. Let's keep startThemeTransition in career.html.
  
  // Remove bottom event listeners
...
console.log("Extraction JS script ready!");
